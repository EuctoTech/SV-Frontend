...
const getfeesArray = async () => {
  try {
      if (newInvoiceClass && newFeesHeading) {
          ...
          // Iterate over the fetched subFeesArray
          subCategory.forEach(subFee => {
              // Search for corresponding sub_heading in fetchSections data
              const matchedSubHeading = subFeesArray.find(item => item.Sub_Fees === subFee.sub_heading);

              if (matchedSubHeading) {
                  // If sub_heading is found, update the amount
                  subFee.amount = matchedSubHeading.Amount;

                  // Check if the priority already exists in subCategory
                  const priorityIndex = parseInt(matchedSubHeading.Priority) - 1; // Convert Priority to index

                  if (priorityIndex !== subCategory.indexOf(subFee)) {
                      // If the priority index is different from the current index, move the item
                      subCategory.splice(subCategory.indexOf(subFee), 1); // Remove the item from current index
                      subCategory.splice(priorityIndex, 0, subFee); // Insert the item at the priority index
                  }
              }
          });

          console.log('updatedSubFeesArray', subCategory);

          // Set the updated subCategory state
          setSubCategory([...subCategory]); // Make sure to create a new array to trigger state update
...
<Col className="col-3">  
           ...
           <Form.Select  name='standard'  value={newInvoiceClass}  onChange={(e) => setNewInvoiceClass(e.target.value)} onClick={() => { getfeesArray();  }}  >
